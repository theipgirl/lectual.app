...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  if (code) {
    ...
    if (error) {
      ...
      //
      // This MUST land on /sign-in/, not "/" — the marketing home reads no
      // searchParams, so an error there is silently swallowed and the user
      // just sees the landing page. /sign-in/ renders the error and a
      // "request a fresh link" form. Trailing slash is required:
      // next.config.ts sets `trailingSlash: true`, so "/sign-in" would take
      // an extra redirect hop that drops nothing but costs a round trip.
      //
