 ...
 *
 * Payload contract written by the backfill (Workstream E of the plan):
 *
 ...
 *     an amount into a number to re-render it, and nothing recomputes a
 *     balance from total − paid. This is a law firm's money: the only honest
 *     thing to show is exactly what the record says. The one numeric read is
 *     `balanceIsOutstanding`, which decides *emphasis only* and never changes
 *     a displayed figure.
 *  2. **Missing and extra keys are survivable.** The backfill runs in parallel
 *     with this code; a row missing `paid`, or carrying keys nobody has
 *     designed a cell for yet, must degrade to an em dash rather than throw.
 */
 ...
 * is what proves the rows belong to the caller's org, exactly as
 * `activityForMatter` does. Never a service-role client.
 *
